...
echo "Copying cron setup script..."
scp setup_parquet_cron_tinkerboard.sh "$REMOTE_HOST:$REMOTE_DIR/setup_parquet_cron.sh"
echo -e "${GREEN}✓${NC} Cron setup script copied"
echo ""

# Make scripts executable on remote
echo "Making scripts executable..."
ssh "$REMOTE_HOST" "chmod +x $REMOTE_DIR/*.py $REMOTE_DIR/setup_parquet_cron.sh"
...
echo ""
echo "3. Set up the cron jobs:"
echo -e "   ${BLUE}./setup_parquet_cron.sh${NC}"
echo ""
echo "4. Test the hourly capture manually:"
echo -e "   ${BLUE}python3 capture_hourly.py --backup-dir /mnt/usb3/tinkerboard/flights/hourly${NC}"
echo ""
echo "Note: The scripts are configured to copy files to /mnt/usb3/tinkerboard/flights/"
echo "      Make sure this directory exists and is writable on tinkerboard."
